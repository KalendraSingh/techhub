...
const Users = () => {
  ...
  const { mutate: mutateUpdateUser, isLoading: isLoadingUpdateUser } =
    useMutation({
      mutationFn: ({ isAdmin,isUser, userId }) => {
        return updateProfile({
          token: userState.userInfo.token,
          userData: { admin: isAdmin,verified: isUser  },
          userId,
        });
      },
      onSuccess: (data) => {
    ...
    const handleUserCheck = (event, userId,user) => {
      const updatedValue = !user.verified;
      console.log("user",updatedValue)
  
      if (window.confirm("Do you want to change this user's verification status?")) {
        mutateUpdateUser({ isUser: updatedValue, userId });
      } else {
        event.target.checked = !updatedValue;
      }
    }
      ...
      {usersData?.data.map((user) => (
        <tr key={user._id}>
          ...
          <td className="px-5 py-5 text-sm bg-white border-b border-gray-200">
            <p className="text-gray-900 whitespace-no-wrap">
              {user.verified ? "✅" : "❌"}
              <button onClick={(event) => handleUserCheck(event, user._id,user)}>{user.verified ? "Unverify" : "Verify"}</button>
            </p>
